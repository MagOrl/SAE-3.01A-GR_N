...
BEGIN 
    IF ((SELECT id_hab FROM PERSONNEL NATURAL JOIN SPECIALISER_EN WHERE id_pers = NEW.id_pers) not in (SELECT id_hab FROM CAMPAGNE NATURAL JOIN PLATEFORME NATURAL JOIN NECESSITER WHERE id_camp = NEW.id_camp))
    THEN 
