...
class Campagne(db.Model):
    ...
    def validate_campagne(self, key, value):
        if key == 'id_pla' and hasattr(self, 'date_deb_camp') and self.date_deb_camp and hasattr(self, 'duree') and self.duree:
            from datetime import datetime
            try:
                date_ajoutee = datetime.strptime(str(self.date_deb_camp), '%Y-%m-%d').date()
                date_fin_ajoutee = date_ajoutee + timedelta(days=self.duree)
                
                campagnes_existantes = Campagne.query.filter_by(id_pla=value).all()
                for camp in campagnes_existantes:
                    if camp.id_camp != getattr(self, 'id_camp', None):  
                        if hasattr(camp, 'date_deb_camp') and camp.date_deb_camp and hasattr(camp, 'duree') and camp.duree:
                            date_deb_exist = datetime.strptime(str(camp.date_deb_camp), '%Y-%m-%d').date()
                            date_fin_exist = date_deb_exist + timedelta(days=camp.duree)
                            
                            if (date_ajoutee <= date_fin_exist and date_fin_ajoutee >= date_deb_exist):
                                raise ValueError('La plateforme est déjà affectée à une autre campagne pendant cette période.')
            except (ValueError, TypeError, AttributeError):
                pass
        
        if key == 'duree' and hasattr(self, 'id_pla') and self.id_pla:
            plateforme = Plateforme.query.get(self.id_pla)
            if plateforme and hasattr(plateforme, 'duree_max') and plateforme.duree_max:
                if value > plateforme.duree_max:
                    raise ValueError('La durée de la campagne dépasse la durée maximale de la plateforme.')
            
            if plateforme and hasattr(plateforme, 'jours_av_mainte') and plateforme.jours_av_mainte is not None:
                if plateforme.jours_av_mainte >= value:
                    plateforme.jours_av_mainte -= value
                else:
                    plateforme.jours_av_mainte = plateforme.inter_mainte - value if hasattr(plateforme, 'inter_mainte') else 0
        
        if key == 'id_budg' and hasattr(self, 'id_pla') and self.id_pla and hasattr(self, 'duree') and self.duree:
            plateforme = Plateforme.query.get(self.id_pla)
            budget = Budget.query.get(value)
            if plateforme and budget and hasattr(plateforme, 'cout_exploi_jour') and hasattr(budget, 'valeur'):
                cout_nouveau = plateforme.cout_exploi_jour * self.duree
                
                cout_total = 0
                campagnes_budget = Campagne.query.filter_by(id_budg=value).all()
                ...
                if cout_total + cout_nouveau > budget.valeur:
                    raise ValueError(f'Insertion impossible, la campagne est hors budget. Budget couvert : {cout_total}/{budget.valeur}')
    ...
    def validate_habilitations_et_conflits(self, key, value):
        if key == 'id_camp' and hasattr(self, 'id_pers') and self.id_pers:
            ...
            if campagne and hasattr(campagne, 'id_pla'):
                ...
                if plateforme:
                    habilitations_requises = [n.id_hab for n in Necessiter.query.filter_by(id_pla=plateforme.id_pla).all()]
                    habilitations_personnel = [s.id_hab for s in SpecialiserEn.query.filter_by(id_pers=self.id_pers).all()]
                    for hab_req in habilitations_requises:
                        if hab_req not in habilitations_personnel:
                            raise ValueError('Le personnel ne possède pas une habilitation requise pour la plateforme.')
            
            if campagne and hasattr(campagne, 'date_deb_camp') and campagne.date_deb_camp and hasattr(campagne, 'duree'):
                from datetime import datetime
                try:
                    date_ajoutee = datetime.strptime(str(campagne.date_deb_camp), '%Y-%m-%d').date()
                    date_fin_ajoutee = date_ajoutee + timedelta(days=campagne.duree)
                    
                    participations_existantes = Participer.query.filter_by(id_pers=self.id_pers).all()
                    for part in participations_existantes:
                        camp_exist = Campagne.query.get(part.id_camp)
                        if camp_exist and hasattr(camp_exist, 'date_deb_camp') and camp_exist.date_deb_camp and hasattr(camp_exist, 'duree'):
                            date_deb_exist = datetime.strptime(str(camp_exist.date_deb_camp), '%Y-%m-%d').date()
                            date_fin_exist = date_deb_exist + timedelta(days=camp_exist.duree)
                            
                            if (date_ajoutee <= date_fin_exist and date_fin_ajoutee >= date_deb_exist):
                                raise ValueError('Le personnel est déjà affecté à une autre campagne pendant cette période.')
                except (ValueError, TypeError, AttributeError):
                    pass  
            
            if campagne and hasattr(campagne, 'id_pla'):
                plateforme = Plateforme.query.get(campagne.id_pla)
                if plateforme and hasattr(plateforme, 'nb_pers_max') and plateforme.nb_pers_max:
                    nb_participants_actuel = Participer.query.filter_by(id_camp=value).count()
                    if nb_participants_actuel >= plateforme.nb_pers_max:
                        raise ValueError('Le nombre maximum de participants pour cette plateforme est atteint.')
